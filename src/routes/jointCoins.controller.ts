...
export default class CommonCoinsController {
  ...
  setupRoutes() {
    ...
    this.router.patch(RouteNames.BySymbol , (req: Request, res: Response) => this.updateCommonCoinAccount(req, res));
  }
  ...
  async updateCommonCoinAccount(req: Request, res: Response){
    ...
    try {
      const updatedCoin = await this.jointCoinsManager.updateJointCoinAccount(req.params.symbol, req.body.accounts);
      res.status(200).send(updatedCoin);
